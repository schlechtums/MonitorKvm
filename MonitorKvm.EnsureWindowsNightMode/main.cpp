...
#define STATUS_PATH L"Software\\Microsoft\\Windows\\CurrentVersion\\CloudStore\\Store\\DefaultAccount\\Current\\default$windows.data.bluelightreduction.bluelightreductionstate\\windows.data.bluelightreduction.bluelightreductionstate"
#define STATE_DATA_LENGTH 44
#define STATE_VALUE_NAME L"Data"
...
bool WriteDataToRegistry(unsigned char* byteArray, DWORD size, bool nightLightState) {
	...
	LPCWSTR subKey = STATUS_PATH;
	LONG err;

	err = RegOpenKeyExW(hKey, subKey, 0, KEY_SET_VALUE, &hKey);
	...
	if (err != ERROR_SUCCESS)
		printf("The %ws subkey could not be opened. Error code: %x\n", subKey, err);
	else {
		err = RegSetValueExW(hKey, L"Data", 0, REG_BINARY, byteArray, size);
...
bool GetNightLightStateData(unsigned char* byteArray, DWORD size) {
	...
	HKEY hKey = HKEY_CURRENT_USER;
	LPCWSTR subKey = STATUS_PATH;
	LPCWSTR pValue = STATE_VALUE_NAME;
	DWORD dataType = 0;
	LONG err;

	err = RegOpenKeyExW(hKey, subKey, 0, KEY_READ, &hKey);

	if (err != ERROR_SUCCESS)
		printf("The %ws subkey could not be opened. Error code: %x\n", subKey, err);
	else {
		err = RegGetValueW(hKey, NULL, pValue, RRF_RT_REG_BINARY, &dataType, byteArray, &size);
...
bool ModifyBrightness(bool nightLightIsOn) {
	...
	HANDLE hDevice = CreateFileW(L"\\\\.\\LCD", GENERIC_READ | GENERIC_WRITE, 0, NULL, OPEN_EXISTING, 0, NULL);
